...
router.put("/:id", async (req, res) => {
  ...
  if (!login || !password || !currentLogin || !currentPassword) {
    return res.status(400).json({ error: "E-mail atual, senha atual, novo e-mail e nova senha são obrigatórios" });
  }
...
// Get all user emails (for forgot password)
router.get("/emails/all", async (req, res) => {
  try {
    const result = await pool.query("SELECT id, login FROM login ORDER BY login ASC");
...
// Reset password by email (no security, just for forgot password flow)
