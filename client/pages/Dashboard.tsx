        ...
        const fetchData = async () => {
          try {
            const [noStockRes, belowMinRes, expiredRes, expiringSoonRes] =
              await Promise.all([
                fetch("http://localhost:3001/api/estoque?filter=noStock").then(
                  (r) => r.json(),
                ),
                fetch("http://localhost:3001/api/estoque?filter=belowMin").then(
                  (r) => r.json(),
                ),
                fetch("http://localhost:3001/api/estoque?filter=expired").then(
                  (r) => r.json(),
                ),
    ...
    useEffect(() => {
      const fetchRecentMovements = async () => {
        try {
          const response = await fetch("http://localhost:3001/api/movimentacoes/medicamentos?days=7");
      ...
      useEffect(() => {
        const fetchProportionData = async () => {
          try {
            const response = await fetch("http://localhost:3001/api/estoque/proporcao");
            const data = await response.json();
            const formatted = [
              {
                name: "Estoque Geral (medicamentos)",
                value: parseFloat(data.medicamentos_geral.toFixed(2)),
                rawValue: data.medicamentos_geral,
              },
              {
                name: "Estoque Individual (medicamentos)",
                value: parseFloat(data.medicamentos_individual.toFixed(2)),
                rawValue: data.medicamentos_individual,
              },
              {
                name: "Insumos",
                value: data.insumos, 
                rawValue: data.insumos,
              },
            ];

            setStockDistribution(formatted);
          } catch (err) {
            console.error("Erro ao carregar proporções de estoque:", err);
          }
        };
      ...
      useEffect(() => {
        const fetchCabinetData = async () => {
          try {
            const response = await fetch("http://localhost:3001/api/estoque?type=armarios");
            const data = await response.json();

            const formatted = data.map((arm: any) => ({
              cabinet: arm.armario_nome || `Armário ${arm.armario_id}`,
              total: Number(arm.total_geral) || 0,
            }));

            setCabinetStockData(formatted);
          } catch (err) {
            console.error("Erro ao carregar dados dos armários:", err);
          }
        };
